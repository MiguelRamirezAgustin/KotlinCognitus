...
import com.example.cognitus.adapter.ProductoAdpater
...
class ProductosActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        tvMsg = findViewById(R.id.tvMsgP)
        rvNoticias = findViewById(R.id.rvProducto)
    ...
    inner class getProducto : AsyncTask<Void, Void, String>() {
        ...
        override fun doInBackground(vararg voids: Void): String {
            ...
            return ApiGetPostHelper.SendParams(getString(R.string.ws_url_producto), sendParams)
        }
        ...
        override fun onPostExecute(results: String?) {
            ...
            try {
                ...
                for (i in 0 until noticiasArray.length()) {
                    ...
                    val prodTit = mItemObject.getString("prod_titulo")
                    val prodPrec = mItemObject.getString("prod_precio")
                    val prodImg = getString(R.string.img_url) + mItemObject.getString("prod_img")
                    val prodId = mItemObject.getString("prod_id")
                    val objetoProducto = ProductoModel(prodDesc, prodTit, prodPrec, prodImg, prodId)
                    listaProductos.add(objetoProducto)
                }


                linearLayoutManager = LinearLayoutManager(applicationContext)
                val rvNoticias = findViewById<RecyclerView>(R.id.rvProducto)
